...
/**
 * 上传面部图像（仅正面一张）
 * 上传字段: image(二进制流) + chrono_age + format
 * @param {string} filePath 本地临时文件路径
 * @param {number} chronoAge 用户实际年龄
 * @returns {Promise<{batch_id: string, status: string}>}
 */
export async function uploadFaceImages(filePath, chronoAge) {
  return uploadFile(
    '/task1/face/upload',
    filePath,
    'image',
    {
      chrono_age: chronoAge,  // 实际年龄 → Task2 使用
      format: 'jpg'
    }
  );
}
